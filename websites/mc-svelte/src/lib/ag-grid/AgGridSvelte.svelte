<script lang="ts" context="module">
  const formatProperty = new Map<string, (value: unknown) => unknown>([...ComponentUtil.BOOLEAN_PROPERTIES.map((prop) => [prop, ComponentUtil.toBoolean] as const), ...ComponentUtil.NUMBER_PROPERTIES.map((prop) => [prop, ComponentUtil.toNumber] as const)]);
</script>

<script lang="ts">
  import 'ag-grid-community/styles/ag-grid.css';
  import 'ag-grid-community/styles/ag-theme-alpine.css';
  import { ComponentUtil, Grid, type GridOptions, type GridParams, type GridReadyEvent, type Module } from 'ag-grid-community';
  import { onMount } from 'svelte';
  import { SvelteFrameworkComponentWrapper, SvelteFrameworkOverrides } from './SvelteFrameworkComponentWrapper';
  ...
  export let excludeChildrenWhenTreeDataFiltering: Options['excludeChildrenWhenTreeDataFiltering'] = undefined;
  // Integrated Charts
  ...
  export let removePivotHeaderRowWhenSingleValueColumn: Options['removePivotHeaderRowWhenSingleValueColumn'] = undefined;
  // Rendering
  ...
  export let suppressMaxRenderedRowRestriction: Options['suppressMaxRenderedRowRestriction'] = undefined;
  // Row Drag and Drop
  ...
  export let groupRemoveLowestSingleChildren: Options['groupRemoveLowestSingleChildren'] = undefined;
  ...
  // export let suppressDragLeaveHidesColumns: Options['suppressDragLeaveHidesColumns'] = undefined;
  // export let suppressRowGroupHidesColumns: Options['suppressRowGroupHidesColumns'] = undefined;
  export let suppressMakeColumnVisibleAfterUnGroup: Options['suppressMakeColumnVisibleAfterUnGroup'] = undefined;
  ...
  export let suppressModelUpdateAfterUpdateTransaction: Options['suppressModelUpdateAfterUpdateTransaction'] = undefined;
  // RowModel: Infinite
  export let datasource: Options['datasource'] = undefined;
  export let cacheOverflowSize: Options['cacheOverflowSize'] = undefined;
  export let maxConcurrentDatasourceRequests: Options['maxConcurrentDatasourceRequests'] = undefined;
  ...
  export let suppressScrollWhenPopupsAreOpen: Options['suppressScrollWhenPopupsAreOpen'] = undefined;
  ...
  export let suppressPreventDefaultOnMouseWheel: Options['suppressPreventDefaultOnMouseWheel'] = undefined;
  ...
  $: if (isClientSide) updateProp('suppressModelUpdateAfterUpdateTransaction', suppressModelUpdateAfterUpdateTransaction);
  // RowModel: Infinite
  ...
  $: if (rowModelType === 'infinite') updateProp('maxConcurrentDatasourceRequests', maxConcurrentDatasourceRequests);
  $: if (rowModelType === 'infinite' || rowModelType === 'serverSide') updateProp('cacheBlockSize', cacheBlockSize);
  $: if (rowModelType === 'infinite' || rowModelType === 'serverSide') updateProp('maxBlocksInCache', maxBlocksInCache);
  $: if (rowModelType === 'infinite' || rowModelType === 'serverSide') updateProp('infiniteInitialRowCount', infiniteInitialRowCount);
  // RowModel: Server-Side
  $: if (rowModelType === 'serverSide') updateProp('serverSideDatasource', serverSideDatasource);
  $: if (rowModelType === 'serverSide') updateProp('serverSideInfiniteScroll', serverSideInfiniteScroll);
  ...
  $: if (rowModelType === 'serverSide') updateProp('blockLoadDebounceMillis', blockLoadDebounceMillis);
  $: if (rowModelType === 'serverSide') updateProp('purgeClosedRowNodes', purgeClosedRowNodes);
  $: if (rowModelType === 'serverSide') updateProp('serverSideSortAllLevels', serverSideSortAllLevels);
  $: if (rowModelType === 'serverSide') updateProp('serverSideFilterAllLevels', serverSideFilterAllLevels);
  $: if (rowModelType === 'serverSide') updateProp('serverSideSortOnServer', serverSideSortOnServer);
  $: if (rowModelType === 'serverSide') updateProp('serverSideFilterOnServer', serverSideFilterOnServer);
  $: if (rowModelType === 'serverSide') updateProp('serverSideInitialRowCount', serverSideInitialRowCount);
  $: if (rowModelType === 'serverSide') updateProp('getChildCount', getChildCount);
  $: if (rowModelType === 'serverSide') updateProp('getServerSideGroupLevelParams', getServerSideGroupLevelParams);
  $: if (rowModelType === 'serverSide') updateProp('isServerSideGroupOpenByDefault', isServerSideGroupOpenByDefault);
  $: if (rowModelType === 'serverSide') updateProp('isApplyServerSideTransaction', isApplyServerSideTransaction);
  ...
  $: if (rowModelType === 'viewport') updateProp('viewportRowModelPageSize', viewportRowModelPageSize);
  $: if (rowModelType === 'viewport') updateProp('viewportRowModelBufferSize', viewportRowModelBufferSize);
  // Scrolling
