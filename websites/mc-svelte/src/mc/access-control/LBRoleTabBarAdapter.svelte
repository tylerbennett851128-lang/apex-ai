<script lang="ts">
  ...
  import { RoleDetailQueryService, sidebarService } from '$mc';
  ...
  const onAddInfo = () => {
    queryService.actionBarEvent.update((value) => (value = new ActionBarEvent(ACTION_BAR_EVENT.ADD)));
  };
  ...
  const deleteItems = () => {
    ...
    if (selectedRows && selectedRows.length > 0) {
      const service = queryService as RoleDetailQueryService;
      const primaryColumn = service.primaryColumn;
      const postfix = service.postfix;

      if (selectedRows.length === 1) {
        message = `${selectedRows[0][primaryColumn]} ${postfix} will be permanently removed from ${service.roleName}.`;
      } else {
        message = `${selectedRows.length} selected ${postfix}s will be permanently removed from ${service.roleName}.`;
      }
    }
  };
...
<LBModal showModal={openDeleteModal} title={'Delete Selected Items'} captionForSubmit={'DELETE'} on:modalAction={onToggleModal} cssModal={'max-w-md rounded-lg'}>
  <div class="flex h-full w-full items-center justify-center p-4">
    <span class="text-md text-gray-800">{message}</span>
  </div>
</LBModal>
