<script lang="ts">
  import LBNumberInput from '$components/LBSubmit/FormComponents/LBNumberInput.svelte';
...
<LBNumberInput readOnly={cell.readOnly} bind:value={row_data[cell.id]} placeholder={`Enter ${cell.name}`} onChange={(value) => onChange(cell.id, value)} />
