<script lang="ts" context="module">
  ...
  export type DuckDBSectionType = {
    title: string;
    url: string;
    lastUrl: string;
    sectionType: 'Detail' | 'Child' | 'Tracker';
    atomic_type: string;
    atomic_key: string;
    version_key: string;
    file_name: string;
...
<script lang="ts">
  ...
  let curSectionData: DuckDBSectionType;
  let mapUrlToSection: Record<string, DuckDBSectionType> = {};
  const baseUrl = '/duckdb';
  const storageKey = 'DuckDBSection';
  ...
  setContext(AtomicSection, {
    // This context is shared between all pages under duckdb folder.
    setMap: (data: DuckDBSectionType) => {
      mapUrlToSection[data.url] = data;
      save_storage(storageKey, mapUrlToSection);
    },

    getSectionData: (url: string): DuckDBSectionType => {
  ...
  <div class="flex w-full flex-row">
    <div class="flex w-[80%] items-center justify-start py-1">
      {#if $page.url.pathname === baseUrl}
        <LBBreadcrum title={'DuckDB Files'} />
